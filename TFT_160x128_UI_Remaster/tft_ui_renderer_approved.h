...
private:
  ...
  void drawTopBar(const Model &m) {
    ...
    centerText(13, m.topTime, u8g2_font_6x12_tf, TftUiTheme::FG, 52, 106);
    textAt(110, 13, String(m.temperatureC) + "C", u8g2_font_5x8_tf, TftUiTheme::FG);
  ...
  void drawPlayerMetadata(const Model &m) {
    centerText(83, m.title, u8g2_font_7x14B_tf, TftUiTheme::FG, 4, 156);
    centerText(96, m.artist, u8g2_font_5x8_tf, TftUiTheme::DIM, 4, 156);
  }
  ...
  void drawBrickEq(int16_t x0, int16_t baseY, uint8_t level, bool mirror) {
    ...
    for (uint8_t c = 0; c < 5; ++c) {
      ...
      for (int r = 0; r < rows; ++r) {
        ...
        _gfx.fillRect(x, y, brickW, brickH, TftUiTheme::eqColorForRow(r, maxRows));
      }
    }
  }
  ...
  void drawClockBody(const Model &m) {
    centerText(70, m.topTime, u8g2_font_logisoso42_tn, TftUiTheme::FG, 18, 142);
    centerText(91, m.date, u8g2_font_7x14B_tf, TftUiTheme::FG, 28, 132);
  }
  ...
  void drawWeatherBody(const Model &m) {
    ...
    centerText(86, String("Feels ") + m.weatherFeelsC + "C", u8g2_font_6x12_tf, TftUiTheme::DIM, 51, 109);
    if (m.humidity >= 0) {
      centerText(67, String(m.humidity), u8g2_font_logisoso24_tn, TftUiTheme::FG, 111, 151);
      textAt(148, 65, "%", u8g2_font_6x12_tf, TftUiTheme::FG);
      centerText(86, "HUMIDITY", u8g2_font_5x8_tf, TftUiTheme::DIM, 109, 160);
    } else {
      centerText(67, "--", u8g2_font_logisoso24_tn, TftUiTheme::DIM, 111, 151);
    }
    centerText(86, weatherLabel(m.weatherState), u8g2_font_6x12_tf, TftUiTheme::DIM, 0, 51);
  }
  ...
  void drawSignedTemperature(int16_t left, int16_t right, int16_t baselineY, int value) {
  ...
  void drawVolumeValue(int percent) {
    ...
    _text.print(digits);
    textAt(startX + digitsW + 5, 85, "%", u8g2_font_7x14B_tf, TftUiTheme::FG);
  }
  ...
  void drawWifiIcon(int16_t x, int16_t y, bool connected) {
    ...
    if (!connected) _gfx.drawLine(x, y + 1, x + 16, y + 14, TftUiTheme::RED);
  }
  ...
  void drawBatteryStatus(int16_t x, int16_t y, const Model &m) {
    ...
    if (m.batteryPresent) {
      ...
      if (fill > 0) _gfx.fillRect(x + 2, y + 2, fill, 5, TftUiTheme::batteryColor(m.batteryPercent));
  ...
  void drawWeatherIcon(int16_t x, int16_t y, const String &state, bool night) {
    if (state == "SUN") {
      ...
      for (int i = 0; i < 3; ++i)
        _gfx.drawLine(x + 9 + i * 9, y + 30, x + 6 + i * 9, y + 36, TftUiTheme::CYAN);
    } else if (state == "SNOW") {
      drawCloud(x, y, TftUiTheme::CLOUD);
      for (int i = 0; i < 3; ++i)
        _gfx.fillCircle(x + 8 + i * 10, y + 32 + (i & 1) * 2, 1, TftUiTheme::FG);
    } else {
      drawCloud(x, y, TftUiTheme::CLOUD);
    }
  }
